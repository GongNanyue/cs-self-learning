...
def cifar10(num_train=None, num_test=None, x_dtype=torch.float32):
    ...
    返回 CIFAR10 数据集，必要时自动下载。
    这个函数还可以对数据集进行子采样。

    输入:
    - num_train: [可选] 从训练集中保留多少样本。
      如果未提供，则保留整个训练集。
    - num_test: [可选] 从测试集中保留多少样本。
      如果未提供，则保留整个测试集。
    - x_dtype: [可选] 输入图像的数据类型。

    返回:
    - x_train: 形状为 (num_train, 3, 32, 32) 的 `x_dtype` 张量。
    - y_train: 形状为 (num_train, 3, 32, 32) 的 int64 张量。
    - x_test: 形状为 (num_test, 3, 32, 32) 的 `x_dtype` 张量。
    - y_test: 形状为 (num_test, 3, 32, 32) 的 int64 张量。
    """
