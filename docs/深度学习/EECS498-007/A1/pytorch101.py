...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    M = x.shape[0]
    idx = torch.tensor(
        list(reversed([i for i in range(M)]))
    )
    y = x[idx, :]
    ##########################################################################
    #                            END OF YOUR CODE                            #
...
def sum_positive_entries(x: Tensor) -> Tensor:
    ...
    Returns:
        ...
        你的输出应该是一个 Python 整数，而不是 PyTorch 标量。

        你的实现不应该修改输入张量，并且不应该使用任何显式的 Python 循环（包括推导式）。 你
        应该只使用一次比较操作和一次索引操作来访问输入张量的数据。

        参数： x: 任意形状的 dtype 为 torch.int64 的张量

        返回值： pos_sum: Python 整数，表示 x 中所有正值的总和
    """
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    在形状为 (B, N, M) 的张量 x 和形状为 (B, M, P) 的张量 y 之间执行批量矩阵乘法。

    此实现应使用单个显式循环遍历批次维度 B 来计算输出。
    ...
    返回：
        z: 形状为 (B, N, P) 的张量，其中形状为 (N, P) 的 z[i] 是形状为 (N, M) 的 x[i] 和
           形状为 (M, P) 的 y[i] 之间矩阵乘法的结果。输出 z 应具有与 x 相同的数据类型。
    """
    z = None
    ###########################################################################
    #                      TODO: Implement this function                      #
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    通过减去均值并除以每列的标准差来归一化矩阵 x 的列。你应该返回一个新的张量；输入不应被修改。

    更具体地说，给定一个形状为 (M, N) 的输入张量 x，生成一个形状为 (M, N) 的输出张量 y，
    其中 y[i, j] = (x[i, j] - mu_j) / sigma_j，其中 mu_j 是列 x[:, j] 的均值。

    你的实现不应使用任何显式的 Python 循环（包括列表推导式）；你只能使用张量的基本算术操作
    （+, -, *, /, **, sqrt）、求和归约函数以及 reshape 操作以促进广播。
    你不应使用 torch.mean、torch.std 或其实例方法变体 x.mean、x.std。

    参数:
        x: 形状为 (M, N) 的张量。

    返回:
        y: 如上所述的形状为 (M, N) 的张量。它应具有与输入 x 相同的数据类型。
    """
    y = None
    ##########################################################################
    #                      TODO: Implement this function                     #
    ...
    sigma = torch.sqrt((torch.sum((x - mu) ** 2, dim=0, keepdim=True)) / (M - 1))
    y = (x - mu) / sigma
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def mm_on_gpu(x: Tensor, w: Tensor) -> Tensor:
    ...
    具体来说，给定两个输入张量，这个函数应该：
    (1) 将每个输入张量移动到 GPU；
    (2) 执行 GPU 张量之间的矩阵乘法；
    (3) 将结果移回 CPU。

    当你将张量移动到 GPU 时，使用 "your_tensor.cuda()" 操作。

    参数：
        x: 形状为 (A, B) 的张量，在 CPU 上
        w: 形状为 (B, C) 的张量，在 CPU 上

    返回：
        y: 如上所述，形状为 (A, C) 的张量。它不应该在 GPU 上。
    """
    y = None
    ##########################################################################
    #                      TODO: Implement this function                     #
